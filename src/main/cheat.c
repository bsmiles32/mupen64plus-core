...
{
    return *(unsigned short *)(((unsigned char*)g_rdram.ram + ((address & 0xFFFFFF)^S16)));
}

static unsigned char read_address_8bit(unsigned int address)
{
    return *(unsigned char *)(((unsigned char*)g_rdram.ram + ((address & 0xFFFFFF)^S8)));
}

static void update_address_16bit(unsigned int address, unsigned short new_value)
{
    *(unsigned short *)(((unsigned char*)g_rdram.ram + ((address & 0xFFFFFF)^S16))) = new_value;
}

static void update_address_8bit(unsigned int address, unsigned char new_value)
{
    *(unsigned char *)(((unsigned char*)g_rdram.ram + ((address & 0xFFFFFF)^S8))) = new_value;
}

static int address_equal_to_8bit(unsigned int address, unsigned char value)
{
    unsigned char value_read;
    value_read = *(unsigned char *)(((unsigned char*)g_rdram.ram + ((address & 0xFFFFFF)^S8)));
...
static int address_equal_to_16bit(unsigned int address, unsigned short value)
{
    ...
    value_read = *(unsigned short *)(((unsigned char*)g_rdram.ram + ((address & 0xFFFFFF)^S16)));
    return value_read == value;
}
