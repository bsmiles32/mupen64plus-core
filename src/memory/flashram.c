...
void dma_read_flashram(void)
{
    ...
    {
    case STATUS_MODE:
        g_rdram.ram[pi_register.pi_dram_addr_reg/4] = (unsigned int)(flashram_info.status >> 32);
        g_rdram.ram[pi_register.pi_dram_addr_reg/4+1] = (unsigned int)(flashram_info.status);
        break;
    case READ_MODE:
        flashram_read_file();
        for (i=0; i<(pi_register.pi_wr_len_reg & 0x0FFFFFF)+1; i++)
        {
            ((unsigned char*)g_rdram.ram)[(pi_register.pi_dram_addr_reg+i)^S8]=
